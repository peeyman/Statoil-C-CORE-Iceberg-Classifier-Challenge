...
from keras.layers import Input,Conv2D,MaxPool2D,concatenate,Flatten,Dense,\
...
def inception_module(X):
    ...
    tower_3 = Conv2D(64, (7, 7), padding='same', activation='relu')(tower_3)

    tower_4 = MaxPool2D((3, 3), strides=(1,1), padding='same')(X)
...
valid_pred_df.to_csv('../Preds/valid.inception.size.v1.csv',index=False)
#Submission for each day.
submission = pd.DataFrame()
submission['id']=test['id']
submission['is_iceberg']=test_preds
submission.to_csv('../Preds/sub.inception.size.v1.csv', index=False)
